...
const AddBoat = () => {
  ...
  useEffect(() => {
    ...
    const userData = JSON.parse(sellerID);
    
    if (sellerID) {
      dispatch({ type: 'SET_SELLER_ID', payload: userData._id });
    }
  ...
  const mutation = useMutation({
    mutationFn: async (boatData) => {
      ...
      return newRequest.post('/yatch/actions/addYatch', boatData, { withCredentials: true });
    },
    onSuccess: () => {
  ...
  const mutation = useMutation({
    mutationFn: async (boatData) => {
      ...
      newRequest.post('/yatch/actions/addYatch', boatData, { withCredentials:true });
